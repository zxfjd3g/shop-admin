<template>
  <div>
    ...
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            v-if="(scope.row.level == 1 || scope.row.level == 2) && $hasBP('permission.add')"
            type="text"
            size="mini"
          ...
          <el-button
            v-if="scope.row.level == 3 &&  $hasBP('permission.add')"
            type="text"
            size="mini"
    ...
    <el-dialog :visible.sync="dialogFormVisible" title="添加子菜单">
      <el-form ref="menu" :model="menu" :rules="menuValidateRules" label-width="120px">
        ...
        <el-form-item label="组件图标" prop="component">
...
export default {
  ...
  methods: {
    fetchNodeList() {
      this.$API.menu.getNestedTreeList().then(response => {
    ...
    remove(data) {
      ...
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        return this.$API.menu.removeById(data.id)
      }).then(() => {
      ...
      this.$refs.permission.validate(valid => {
        if (valid) {
          if (this.permission.id) {
            ...
            this.$API.menu.saveLevelOne(this.permission).then(response => {
    ...
    appendLevelOne() {
      this.$API.menu.saveLevelOne(this.menu)
        .then(response => {
    ...
    update(obj) {
      this.$API.menu.update(obj).then(response => {
        this.dialogFormVisible = false
        this.$message({
          type: 'success',
          message: '修改成功'
